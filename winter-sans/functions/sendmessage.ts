...
export default SlackFunction(
  ...
  async ({ inputs, client }) => {
    ...
    const airesponse1 = await fetch(`https://api.cloudflare.com/client/v4/accounts/${"de299eff7ceaa5006bd30245bd9a6c77"}/ai/run/${"@cf/meta/llama-3.1-8b-instruct"}`, {
      ...
      body: JSON.stringify({
        messages: [
          { role: "user", content: `The user is a Hack Club member who has spent ${hoursSlackdec} hours on Slack messaging and ${hoursdecimal} hours coding today. Could you provide some comments fitting Sans (from Undertale) in winter season. Make some bad winter jokes, some bad puns, and just be Sans. Try to encourage the user to either do better or to congratulate them and help them spend more time on Hackatime. Please provide a short 100-word response.`}
        ],
        max_tokens: 300, 
        temperature: 0.8,
      }),
