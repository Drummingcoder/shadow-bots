  ...
  async ({ inputs, client }) => {
    ...
    if (getmyResp.item.responded && inputs.user != "U0A09UZK4K0") {
      ...
      const airesponse1 = await fetch(`https://api.cloudflare.com/client/v4/accounts/${"de299eff7ceaa5006bd30245bd9a6c77"}/ai/run/${"@cf/meta/llama-3.1-8b-instruct"}`, {
        ...
        body: JSON.stringify({
          messages: [
            { role: "user", content: `The user is a Hack Club member who said this about their day: ${inputs.message}. Could you make a story fitting Sans (from Undertale) in winter season that tries to comfort them if they're having a bad day or have fun if they're having a good day? Try to make the user happier. Make some bad jokes, some bad puns, just be Sans. Please provide around a 300 word response without any headers or titles.`}
          ],
          max_tokens: 500, 
          temperature: 0.8,
        }),
